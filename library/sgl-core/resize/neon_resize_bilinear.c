...
static void sgl_neon_resize_bilinear_line_stripe(void *current, void *cookie);

sgl_result_t sgl_neon_resize_bilinear(
                ...
                int32_t bpp)
{
    ...
    if (errcnt == 0) {
        ...
        if (lut == NULL) {
            ...
            temp_lut = sgl_neon_create_bilinear_lut(d_width, d_height, s_width, s_height);
            lut = temp_lut;
        }
        ...
        if (lut != NULL) {
            ...
            if (pool == NULL) {
                ...
                for (cur.row = 0; cur.row < d_height; ++cur.row) {
                    sgl_neon_resize_bilinear_line_stripe((void *)&cur, (void *)&data);
                }
            }
            else {
                ...
                if ((operations != NULL) && (currents != NULL)) {
                    ...
                    sgl_threadpool_attach_routine(pool, sgl_neon_resize_bilinear_line_stripe, operations, (void *)&data);
            ...
            if (temp_lut != NULL) {
                ...
                sgl_neon_destroy_bilinear_lut(temp_lut);
            }
        }
    }
    else {
